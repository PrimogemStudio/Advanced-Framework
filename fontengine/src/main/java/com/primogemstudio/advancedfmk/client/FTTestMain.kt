...
fun main() {
    ...
    oplist.forEach {
        when (it.type) {
            OpType.MOVE -> s.append(String.format("M%.2f %.2f", it.target.x, it.target.y))
            OpType.LINE -> s.append(String.format("L%.2f %.2f", it.target.x, it.target.y))
            OpType.CONIC -> s.append(String.format("Q%.2f %.2f %.2f %.2f", it.target.x, it.target.y, it.control1?.x, it.control1?.y))
            OpType.CUBIC -> s.append(String.format("C%.2f %.2f %.2f %.2f %.2f %.2f", it.target.x, it.target.y, it.control1?.x, it.control1?.y, it.control2?.x, it.control2?.y))
        }
    }
    println(s)
}