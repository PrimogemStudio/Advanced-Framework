...
class SimulateResultBinaryFileOutputStream(out: OutputStream): NBTOutputStream(out) {
    ...
    fun recStatus() = logger.info("${Ansi.ansi().fgBrightBlue()}$targetNum${Ansi.ansi().reset()}/${Ansi.ansi().fgBrightYellow()}~${(targetNum.toDouble() / processed).toLong()}${Ansi.ansi().reset()}, ${Ansi.ansi().fgBrightBlue()}${processed * 100} %${Ansi.ansi().reset()} -> ${Ansi.ansi().fgBrightGreen()}${processed_suc * 100} % / ${Ansi.ansi().fgBrightRed()}${processed_fai * 100} % ${Ansi.ansi().reset()}")
